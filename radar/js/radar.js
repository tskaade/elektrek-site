...
async function fetchFlights(){
  ...
  const radiusNm = Math.min(CFG.maxRadiusNm, Math.ceil(rangeKm/1.852));
  const url=`${CFG.apiBase}/point/${user.lat.toFixed(4)}/${user.lon.toFixed(4)}/${radiusNm}`;
  try{
    const res = await fetch(url,{cache:'no-store',headers:{'Accept':'application/json'}});
    if(!res.ok) throw new Error('HTTP '+res.status);
    const data = await res.json();
    const ac = data.ac || data.aircraft || [];
    flights = ac.filter(a=>a.lat!=null && a.lon!=null && (a.alt_baro!=='ground'))
      .map(a=>{
    ...
    const d=new Date();
    lastUpdLabel = d.toTimeString().slice(0,8);
    lastupdEl.textContent = lastUpdLabel;
    setStatus(null);
  }catch(e){
    if(flights.length===0) setStatus('NO FLIGHT FEED — RETRYING…');
