...
## Origin / destination (route lookup)

ADS-B aircraft don't broadcast where they came from or are going — that's not in
the signal. adsb.lol has a route endpoint, but testing via
`route-diagnostic.html` showed it **doesn't work from the browser** (no CORS on
the POST), so route lookup ships **disabled** (`route.enabled:false` in config).

The code path is left in place: if you find a route source that works from the
browser, point `route.url` at it and flip `enabled:true`. The card will then
show **ORIGIN → DESTINATION** when you tap an aircraft. For authoritative routing
you'd need a paid keyed service (FlightLabs, AirLabs).

## Compass smoothing

The raw magnetometer is noisy and occasionally throws wild readings, which made
the scope twitch. The compass now filters input in two stages: implausible jumps
(default >35° in one reading) are rejected unless several consecutive readings
confirm a real turn, and the displayed heading eases slowly toward the validated
value with a deadband and slew-rate cap. Result: steady when still, smooth on
real turns (settles in ~1-2s), and immune to one-off spikes.

Tune it in `js/config.js` under `compass`: raise `smoothing` (toward 0.95) for
calmer/slower, lower it for snappier; raise `outlierConfirmCount` to reject
jumps harder.
