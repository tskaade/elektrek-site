...
/* ---------- flight data (multi-source, CORS-tolerant) ---------- */
let activeSource = null;     // remembered last-working source
...
async function tryFetch(src, lat, lon, r){
  const url = src.url
...
async function fetchFlights(){
  ...
  // try the last-working source first, then the rest in order
  const ordered = activeSource
    ? [activeSource, ...CFG.sources.filter(s=>s!==activeSource)]
    : CFG.sources.slice();
  let lastErr = null;
  for(const src of ordered){
    try{
      const ac = await tryFetch(src, user.lat, user.lon, r);
  ...
  if(flights.length===0){
    const why = (lastErr && /abort/i.test(lastErr.message)) ? 'TIMEOUT' : 'BLOCKED';
